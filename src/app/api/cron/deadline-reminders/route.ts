...
export async function GET(req: Request) {
  ...
  for (const stage of stages) {
    ...
    for (const t of tasks as any[]) {
      ...
      // The admin who assigned it gets reminded too, alongside every assignee.
      const recipients = Array.from(new Set([...assigneeIds, t.assignedBy.toString()]));

      await Notification.insertMany(
        recipients.map((id: string) => ({ recipient: id, message, type: "deadline", taskId: t._id }))
      );
      waitUntil(sendPushToUsers(recipients, { title: "Deadline approaching", body: message, url: "/dashboard" }));
